...
- `scripts/Invoke-WorkspacePreflight.ps1` 是静态预检入口，只检查正式目录里的空目录和禁用旧入口；它不启动 Unity、不修改 Library、不修复资产数据库。
