...
namespace Puerts.Editor.Generator
{
    ...
    public static class CSharpFileExporter
    {
        ...
        public static void GenReflectionModeNativeFiles(string saveTo)
        {
            saveTo = EnsureDirectoryPath(saveTo);
            Directory.CreateDirectory(saveTo);

            var emptySignatureInfos = new List<CSharpCodeGen.SignatureInfo>();
            WriteFile(Path.Combine(saveTo, "PuertsIl2cppWrapper.cpp"), CSharpCodeGen.GenWrapperFile(emptySignatureInfos));
            WriteFile(Path.Combine(saveTo, "PuertsValueType.h"), CSharpCodeGen.GenValueTypeFile(new List<CSharpCodeGen.ValueTypeInfo>()));
            WriteFile(Path.Combine(saveTo, "PuertsIl2cppFieldWrapper.cpp"), CSharpCodeGen.GenFieldWrapperFile(emptySignatureInfos));
            WriteFile(Path.Combine(saveTo, "PuertsIl2cppBridge.cpp"), CSharpCodeGen.GenBridgeFile(GetSharedBridgeInfos()));
            DeleteGeneratedWrapperDefFiles(saveTo);
            CopyStaticResources(saveTo);
            GenMacroHeader(saveTo);
        }
            ...
            foreach (var method in methodToWrap)
            {
                GenericArgumentInInstructions(method, typeInGenericArgument, processed, mb =>
                {
                    try
                    {
                        ...
                        return GetReferencedMethods(mb);
                    }
                    catch (Exception e)
                    {
...
#pragma warning disable CS0618
                var usingDecls = allTypeMayContainUsing.SelectMany(t =>
                    ...
                    mb =>
                    {
                        try { return GetReferencedMethods(mb); }
                        catch { }
                        return Enumerable.Empty<MethodBase>();
                    }).Where(o => o is MethodInfo)
...
#pragma warning restore CS0618
            ...
            bridgeInfos.AddRange(GetSharedBridgeInfos());
        ...
        private static List<CSharpCodeGen.SignatureInfo> GetSharedBridgeInfos()
        {
            var bridgeInfos = new List<CSharpCodeGen.SignatureInfo>();
            var parameterSignatures = new List<string>();
            string voidSignature = TypeUtils.GetTypeSignature(typeof(void));
            string objectSignature = TypeUtils.GetTypeSignature(typeof(Console));

            for (int i = 0; i < 10; ++i)
            {
                bridgeInfos.Add(new CSharpCodeGen.SignatureInfo()
                {
                    Signature = voidSignature + string.Join("", parameterSignatures),
                    CsName = "Action_Shared_" + parameterSignatures.Count,
                    ReturnSignature = voidSignature,
                    ThisSignature = null,
                    ParameterSignatures = parameterSignatures.ToList()
                });
                bridgeInfos.Add(new CSharpCodeGen.SignatureInfo()
                {
                    Signature = objectSignature + string.Join("", parameterSignatures),
                    CsName = "Func_Shared_" + parameterSignatures.Count,
                    ReturnSignature = objectSignature,
                    ThisSignature = null,
                    ParameterSignatures = parameterSignatures.ToList()
                });
                parameterSignatures.Add(objectSignature);
            }
        ...
        private static void DeleteGeneratedWrapperDefFiles(string saveTo)
        {
        ...
        private static IEnumerable<MethodBase> GetReferencedMethods(MethodBase method)
        {
            ...
            for (var offset = 0; offset < il.Length;)
            {
                ...
                try
                {
                    ...
                    MemberInfo member = opCode.OperandType == OperandType.InlineMethod
        ...
        private static OpCode ReadOpCode(byte[] il, ref int offset)
        {
            ...
            return offset < il.Length ? TwoByteOpCodes[il[offset++]] : default(OpCode);
        }
        ...
        private static int GetOperandSize(OperandType operandType, byte[] il, int offset)
        {
            switch (operandType)
            {
                ...
                case OperandType.InlineSwitch:
                    return offset + 4 <= il.Length ? 4 + BitConverter.ToInt32(il, offset) * 4 : 0;
                default:
                    return 0;
            }
        }
