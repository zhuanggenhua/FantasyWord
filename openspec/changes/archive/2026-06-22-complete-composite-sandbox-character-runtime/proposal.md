...
- `GAS 完整合同` 不是可以长期停留在“部分接入、部分 legacy”状态的候选，而是复杂开放世界角色规则层必须完成的正式替换。
...
- 把 `GAS 完整合同` 从“属性读取已优先走 ASC，但仍有 legacy 过渡壳”推进到明确替换边界：属性、能力规则、持续效果和存读档恢复不再长期维持双轨。
...
- GAS 规则层与 legacy 属性/效果/能力闭包的最终替换边界。
