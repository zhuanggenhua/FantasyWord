...
## Purpose
定义 `FantasyWord` 当前生效的基础运行时规范：项目级运行时入口、系统生命周期、数据库与命令合同、事件派发机制，以及 foundation 阶段已经裁决的单一真相边界。
## Requirements
...
- **WHEN** a formal equipment owner equips or unequips equipment
- **THEN** the runtime sends the corresponding GameCore `Equipment*` event through Yoki `EventKit.Type`
- **WHEN** a formal character ability owner gains or loses an ability
- **THEN** the runtime sends the corresponding GameCore `CharacterAbility*` event through Yoki `EventKit.Type`
...
#### Scenario: Chosen reference owners become the only same-duty truth source

- **WHEN** a reference-aligned runtime owner has been selected for a responsibility such as character abilities, character inventory, character equipment, or character weapon handling
- **THEN** same-duty serialized fields, event hosts, and query/command entry points do not remain duplicated on callers, UI panels, or temporary helper hosts
- **AND** optional null-resolution or fallback paths are implemented inside that same formal owner
- **AND** the project does not keep `Profile`, `fallback`, adapter, wrapper, or parallel compatibility layers as a second same-duty entry
