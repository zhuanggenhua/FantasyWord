...
$resultRelativePath = "Temp/UnityBridge/results/composite-runtime-smoke.json"
...
foreach ($requiredTrackedPath in @(
    ...
    if (-not (Test-IsGitTrackedFile -ProjectRootPath $ProjectRoot -RelativePath $requiredTrackedPath)) {
        throw "Composite runtime smoke refuses to use untracked candidate validation scene asset: $requiredTrackedPath"
    }
}
...
if ($openedScenes.Count -ne 1) {
    throw "Composite runtime smoke requires a single opened scene. Current count: $($openedScenes.Count)."
}

$originalScenePath = [string]$openedScenes[0].path
if ([bool]$openedScenes[0].isDirty) {
    throw "Opened scene [$originalScenePath] is dirty. Save or discard changes before running composite runtime smoke."
}
...
try {
    $lockResponse = Invoke-BridgeJson -ToolName "scene-lock-acquire" -JsonLiteral (@{
            owner = "codex-composite-runtime-smoke"
            reason = "control-group / RTS-order / GAS candidate validation smoke"
            mode = "wait"
            timeoutSeconds = 600
        } | ConvertTo-Json -Compress -Depth 10)
...
public class Script
{
    ...
    {
        return CompositeRuntimeSmokeValidator.Start();
    }
}
'@
    ...
    if ([string]::IsNullOrWhiteSpace($validatorResultPath)) {
        throw "Composite runtime smoke did not return a result file path."
    }

    $validatorResult = Wait-ResultFile -Path $validatorResultPath -TimeoutSeconds $ResultPollTimeoutSeconds

    Write-Host "Composite runtime smoke finished."
    ...
    if (-not $validatorResult.Success) {
        throw "Composite runtime smoke failed: $($validatorResult.Message)"
    }
}
finally {
    if ($startedPlayMode) {
