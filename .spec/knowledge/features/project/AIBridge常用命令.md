---
name: AIBridge常用命令
description: 项目知识：AIBridge常用命令.md：AIBridge常用命令。
metadata:
  type: doc
  status: 已交付
---
...
- 只记录当前真实存在的正式入口，不记录已删除或已归档的废弃入口。
...
- 能力边界：
  - 对正式 EX-GAS 技能，只检查旧 `AbilitySheet` / 旧执行资产字段是否退回兼容残留
...
- `Success = true`
  - 对正式技能，不代表 `AbilitySheet -> AbilityExecutionAsset` 是正式链路；正式链路必须回到 EX-GAS Ability / Timeline / GameplayEffect / Cue
