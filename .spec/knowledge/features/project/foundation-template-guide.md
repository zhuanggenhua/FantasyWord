---
name: foundation-template-guide
description: 项目知识：foundation-template-guide.md：foundation-template-guide。
metadata:
  type: doc
  status: 已交付
---
...
- `scripts/Export-FoundationTemplate.ps1`
  - 导出当前地基模板。
  - `core` 导出 AI 规范入口、项目本地 skill、`GameCore` 参考直拷层、当前 `Assets/Editor/GameCore/Tests` 里的 EditMode 测试、门禁、`.spec/knowledge/features/project` 与 foundation OpenSpec。
...
- 当前 `.spec/knowledge/features/project` 与 foundation OpenSpec
