---
name: AbilitySheet执行资产编辑器缓存不同步排查记录
description: 项目知识：bugs/AbilitySheet执行资产编辑器缓存不同步排查记录.md：AbilitySheet执行资产编辑器缓存不同步排查记录。
metadata:
  type: doc
  status: 已交付
---
...
- 旧 `AbilitySheet` 资产不得残留 `m_formalGasAbilityCode`；正式或已用 EX-GAS 表表达的能力不能再保留同职责旧表资产入口。
