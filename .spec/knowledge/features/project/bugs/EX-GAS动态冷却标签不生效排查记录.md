---
name: EX-GAS动态冷却标签不生效排查记录
description: 项目知识：bugs/EX-GAS动态冷却标签不生效排查记录.md：EX-GAS动态冷却标签不生效排查记录。
metadata:
  type: doc
  status: 已交付
---
...
- `MeleeAttackAbilityEditModeTests` 全类必须通过，确认基础攻击命中、mana cost、cooldown 与废弃入口收口没有互相打坏。
