...
1. **原生能力优先**：Unity 或当前依赖已经提供的正式资产、组件、API、导入管线和序列化语义优先使用；只有原生能力无法表达项目需求时，才新增包装层、生成器或自定义资产类型。
2. **职责内核优先于表面结构**：参考工程只能先提炼它解决的职责边界、数据 owner、刷新/保存语义和验收位置；不得只因为参考工程有某个类、字段、目录或命名，就在本项目复制一层同形结构。
3. **最薄可行抽象**：能用现有 owner 加一个显式字段解决的，不新增独立 ScriptableObject、manager、桥接层或全局服务；只有当它有独立生命周期、跨系统复用、单独校验、热切换或内容生产闭包时，才升级为独立资产或模块。
4. **单一真相源**：每类数据只能有一个现实 owner。作者源数据、派生生成物、运行时状态、编辑器配置和验证报告不得互相代持；生成器不得把派生结果写回不该拥有它的源数据。
5. **约束匹配优先于“更通用”**：方案必须匹配当前阶段、素材形态、运行时入口、现有保存/刷新语义和用户已确认目标；不得为了显得架构完整而引入暂时不用的通用层。
6. **证据覆盖边界**：每个参考只能证明它实际覆盖的事实。外部源码、官方 API、当前项目代码、历史文档和测试结果必须分别标明，不能混成一个“参考后就是最佳实践”的结论。
