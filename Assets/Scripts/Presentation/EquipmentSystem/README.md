...
这是 `FantasyWord` 的正式像素风角色换装表现模块。它只负责角色外观、装备层渲染、动画帧数据和编辑器处理工具；背包、掉落、物品使用、存档和玩家控制器接线仍由后续正式玩法系统负责。

核心特点：

- **SpriteLibrary 动画变体**：所有角色共用纯动作 Animator 状态机和动作时序；角色与方向差异由工作台角色选项或运行时驱动直接持有的 SE/SW/NE/NW 四向 `SpriteLibraryAsset` 提供，共享动画片段只驱动不含方向的 `SpriteResolver.m_SpriteKey`。
...
// 获取主手武器
public EquipmentRenderData GetMainHandWeapon()

// 获取副手武器
public EquipmentRenderData GetOffHandWeapon()
