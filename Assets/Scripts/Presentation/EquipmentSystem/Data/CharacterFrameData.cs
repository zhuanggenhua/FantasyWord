...
{
    [NonSerialized]
    Dictionary<long, FrameData> _frameLookup;

    [NonSerialized]
    int _frameLookupSourceCount = -1;
    ...
    {
        EnsureFrameLookup();
        return _frameLookup != null && _frameLookup.TryGetValue(GetFrameLookupKey(frameIndex, rowIndex), out FrameData frameData)
            ? frameData
            : null;
    }
    ...
    public FrameData GetOrCreateFrame(int frameIndex, int rowIndex)
    {
        ...
        if (f == null)
        {
            ...
            frames.Add(f);
            _frameLookupSourceCount = -1;
        }
        return f;
    }

    void EnsureFrameLookup()
    {
        if (_frameLookup != null && _frameLookupSourceCount == frames.Count)
            return;

        if (_frameLookup == null)
            _frameLookup = new Dictionary<long, FrameData>();
        else
            _frameLookup.Clear();
        ...
        for (int i = 0; i < frames.Count; i++)
        {
            ...
            long key = GetFrameLookupKey(frameData.frameIndex, frameData.rowIndex);
            if (!_frameLookup.ContainsKey(key))
                _frameLookup.Add(key, frameData);
        }

        _frameLookupSourceCount = frames.Count;
    }

    static long GetFrameLookupKey(int frameIndex, int rowIndex)
    {
        return ((long)rowIndex << 32) ^ (uint)frameIndex;
    }
}
...
{
    [NonSerialized]
    Dictionary<AnimationTypeItem, AnimationData> _animationByType;

    [NonSerialized]
    Dictionary<string, AnimationData> _animationByKey;

    [NonSerialized]
    int _animationLookupSourceCount = -1;
    ...
    public AnimationData GetAnimation(AnimationTypeItem animType)
    {
        ...
        EnsureAnimationLookup();
        return _animationByType != null && _animationByType.TryGetValue(animType, out AnimationData animationData)
            ? animationData
            : null;
    }
    ...
    public AnimationData GetAnimationByKey(string key)
    {
        ...
        EnsureAnimationLookup();
        return _animationByKey != null && _animationByKey.TryGetValue(key, out AnimationData animationData)
            ? animationData
            : null;
    }
    ...
    void EnsureAnimationLookup()
    {
        if (_animationByType != null
            && _animationByKey != null
            && _animationLookupSourceCount == animations.Count)
        {
            return;
        }

        if (_animationByType == null)
            _animationByType = new Dictionary<AnimationTypeItem, AnimationData>();
        else
            _animationByType.Clear();

        if (_animationByKey == null)
            _animationByKey = new Dictionary<string, AnimationData>(StringComparer.OrdinalIgnoreCase);
        else
            _animationByKey.Clear();

        for (int i = 0; i < animations.Count; i++)
        {
            AnimationData animationData = animations[i];
            if (animationData == null || animationData.animationType == null)
                continue;

            if (!_animationByType.ContainsKey(animationData.animationType))
                _animationByType.Add(animationData.animationType, animationData);

            string key = animationData.animationType.name;
            if (!string.IsNullOrWhiteSpace(key) && !_animationByKey.ContainsKey(key))
                _animationByKey.Add(key, animationData);
        }

        _animationLookupSourceCount = animations.Count;
    }

    void ClearRuntimeLookups()
    {
        _animationLookupSourceCount = -1;
        _animationByType?.Clear();
        _animationByKey?.Clear();
    }
}
