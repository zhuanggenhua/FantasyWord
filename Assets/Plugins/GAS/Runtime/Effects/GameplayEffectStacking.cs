...
namespace GAS.Runtime
{
    ...
    {
        [Label("NeverRefresh - 不刷新Effect的持续时间")]
    ...
    {
        [Label("NeverReset - 不重置Effect的周期计时")]
        NeverRefresh, //不重置Effect的周期计时

        [Label("ResetOnSuccessfulApplication - 每次apply成功后重置Effect的周期计时")]
        ResetOnSuccessfulApplication //每次apply成功后重置Effect的周期计时
    }

    public enum ExpirationPolicy
    {
        [Label("ClearEntireStack - 持续时间结束时, 清除所有层数")]
        ClearEntireStack, //持续时间结束时,清除所有层数

        [Label("RemoveSingleStackAndRefreshDuration - 持续时间结束时减少一层，然后重新经历一个Duration")]
        RemoveSingleStackAndRefreshDuration, //持续时间结束时减少一层，然后重新经历一个Duration，一直持续到层数减为0

        [Label("RefreshDuration - 持续时间结束时,再次刷新Duration")]
    ...
    public sealed class GameplayEffectStackingConfig
    {
        ...
        [Label(GASTextDefine.LABEL_GE_STACKING_TYPE)]
                public StackingType stackingType;
        [HideIf("IsNoStacking")]
        [Label(GASTextDefine.LABEL_GE_STACKING_CODENAME)]
                public string stackingCodeName;
        [Label(GASTextDefine.LABEL_GE_STACKING_COUNT)]
        [HideIf("IsNoStacking")]
                        [ValidateInput("IsLimitCountValid", "必须>=0")]
        public int limitCount;
        [Label(GASTextDefine.LABEL_GE_STACKING_DURATION_REFRESH_POLICY)]
        [HideIf("IsNoStacking")]
        [InfoBox(GASTextDefine.LABEL_GE_STACKING_DENY_OVERFLOW_APPLICATION+"为True时多余的Apply不会刷新Duration", EInfoBoxType.Normal)]
        public DurationRefreshPolicy durationRefreshPolicy;
        [Label(GASTextDefine.LABEL_GE_STACKING_PERIOD_RESET_POLICY)]
        [HideIf("IsNoStacking")]
        public PeriodResetPolicy periodResetPolicy;
        [Label(GASTextDefine.LABEL_GE_STACKING_EXPIRATION_POLICY)]
        [HideIf("IsNoStacking")]
        public ExpirationPolicy expirationPolicy;

        // Overflow 溢出逻辑处理
        [Label(GASTextDefine.LABEL_GE_STACKING_DENY_OVERFLOW_APPLICATION)]
        [HideIf("ShouldHideDenyOverflowApplication")]
        public bool denyOverflowApplication;
                [Label(GASTextDefine.LABEL_GE_STACKING_CLEAR_STACK_ON_OVERFLOW)]
        [ShowIf("IsDenyOverflowApplication")]
        public bool clearStackOnOverflow;
                [Label(GASTextDefine.LABEL_GE_STACKING_CLEAR_OVERFLOW_EFFECTS)]
        [HideIf("IsNoStacking")]
        ...
        public bool IsLimitCountValid() => limitCount >= 0;

        public bool ShouldHideDenyOverflowApplication() => IsNoStacking() || IsNeverRefreshDuration();
