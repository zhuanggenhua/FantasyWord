...
namespace GAS.Editor
{
    ...
    {
        private static bool _runtimeTypeCacheInitialized;

        internal static void EnsureRuntimeTypeCacheInitialized()
        {
            if (_runtimeTypeCacheInitialized)
            {
                return;
            }
            ...
            try
            {
                GAS.General.ReflectionHelper.InvokeStaticMethod("GAS.Runtime.XLauncher", "InitCache");
                _runtimeTypeCacheInitialized = true;
            }
            catch (Exception e)
            {
                Debug.LogWarning($"EX-GAS 编辑器预览初始化类型注册失败，时间轴里的目标捕获预览可能无法显示：{e.Message}");
            }
        }
        ...
        {
            EnsureRuntimeTypeCacheInitialized();
