...
namespace JKFrame.Editor
{
    ...
    public class ExcelAndSoConvertSetting : ConfigBase
    {
        ...
        #region 生成Excel模板
        [Label("生成路径")] public string generateExcelTemplatePath;
        [Label("文件名(空则使用class名)")] public string fileName;
        [Label("生成类型")] public Type generateExcelTemplateClassType;
        [Label("使用Naughty Label进行备注")] public bool useLableText;
        [Label("List<T>集合模式"), Tooltip("用于一张表对应一个SO集合的方式")] public bool useListFied;
        [Label("List<T>字段名称"), ShowIf("CheckGenerateExcelTemplateFieldName")] public string generateExcelTemplateFieldName;
            ...
            using (ExcelPackage excelPackage = new ExcelPackage(fileInfo))
            {
                ...
                for (int i = 0; i < fieldInfos.Length; i++)
                {
                    ...
                    if (useLableText)
                    {
                        ...
                        if (labelAttribute != null) mainSheet.Cells[remarkIndex, cellIndex].Value = labelAttribute.Label;
                    }
                    // 标记变量名
        ...
        {
            [Label("一个Excel生成一个SO")] Excel2OneSO,
            [Label("一个Excel生成多个SO")] Excel2SOs,
            [Label("一个SO生成一个Excel")] OneSo2Excel,
            [Label("多个SO生成一个Excel")] SOs2Excel,
        }

        [InfoBox("互转必须格式符合，1个SO生成一个Excel的前提是这个SO包含一个List")]
        [Label("转换方式")] public Conversion conversion;
        [Label("要转的类")][ShowIf("CheckConversionTye")] public Type conversionTye;
        [InfoBox("True:使用GUID进行资源记录（也就是资源位置不影响表格），False:使用路径进行记录")]
        [Label("GUIDMode")] public bool guidMode;
        ...
        [Label("SO文件名采用前缀_数字的形式"), ShowIf("CheckUseSoFileNamePrefix")] public bool useSoFileNamePrefix;
        [Label("SO文件名使用第几个字段"), ShowIf("CheckSoFileNameUseFileIndex")] public int soFileNameUseFileIndex = 0;
        [Label("SO文件名前缀"), ShowIf("CheckSoFileNamePrefixName")] public string soFileNamePrefixName;

        [Label("SO文件"), ShowIf("CheckSoFile")] public ScriptableObject soFile;

        [Label("SO中List变量的名称"), ShowIf("CheckListFieldName")] public string listFieldName;
        [Label("转换前清空SO文件夹目录"), ShowIf("CheckDeleteFolder")] public bool deleteFolder;
