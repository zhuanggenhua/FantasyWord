...
namespace JKFrame
{
    ...
    public static class PoolSystem
    {
        ...
        private static bool sceneLifecycleRegistered;

        /// <summary>
        /// 对象池是否已经初始化；项目侧集成代码用它避免重复创建池根节点。
        /// </summary>
        public static bool IsInitialized => GameObjectPoolModule != null && ObjectPoolModule != null && poolRootTransform != null;

        /// <summary>
        /// C# 对象池是否已经初始化；纯对象池不需要创建场景 GameObject 根节点。
        /// </summary>
        public static bool IsObjectPoolInitialized => ObjectPoolModule != null;

        /// <summary>
        /// 确保对象池系统可用；允许项目侧在没有手动放置 JKFrameRoot 的测试场景中仍通过正式池接口工作。
        /// </summary>
        public static void EnsureInitialized()
        {
            if (poolRootTransform == null)
            {
                ...
                poolRoot.AddComponent<PoolRootLifecycle>();
            ...
            if (JKFrameRoot.RootTransform != null && poolRootTransform.parent != JKFrameRoot.RootTransform)
            {
        ...
        /// <summary>
        /// 查询指定 GameObject 池当前是否真实存在。
        /// 仅靠调用方的静态“初始化过一次”标记并不足以代表池仍在，
        /// 因为无 JKFrameRoot 的场景切换会重建 PoolSystem 模块。
        /// </summary>
        public static bool HasGameObjectPool(string keyName)
        {
            return GameObjectPoolModule != null
                && !string.IsNullOrEmpty(keyName)
                && GameObjectPoolModule.poolDic.ContainsKey(keyName);
        }
        ...
        {
            if (GameObjectPoolModule == null && ObjectPoolModule == null)
            {
                return;
            }
        ...
        {
            if (poolRootTransform == null || JKFrameRoot.RootTransform != null)
            {
                return;
            }

            GameObject poolRoot = poolRootTransform.gameObject;
            ResetModules();
            if (poolRoot != null)
            {
        ...
        private static void EnsureObjectPoolInitialized()
        {
            ObjectPoolModule ??= new ObjectPoolModule();
        }

        private sealed class PoolRootLifecycle : MonoBehaviour
        {
            private void OnDestroy()
            {
                if (poolRootTransform == transform)
                {
                    ResetModules();
                }
            }
